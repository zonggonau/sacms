...
export function FieldTypeSelector({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-hidden flex flex-col p-0 border border-border shadow-none rounded-none bg-[#f6f6f9]">
        
        {/* Header */}
        <DialogHeader className="px-8 py-6 bg-white border-b shrink-0 flex flex-row items-center justify-between">
          ...
          <div className="relative w-64">
            ...
            <Input 
              ...
              className="pl-9 h-10 bg-[#f6f6f9] border-transparent hover:border-slate-300 rounded-none text-sm font-medium focus-visible:ring-primary focus-visible:bg-white transition-all shadow-none"
            />
          </div>
        </DialogHeader>
                    ...
                    {types.map(ft => {
                      ...
                      return (
                        <button
                          ...
                          className="flex items-center gap-4 p-4 rounded-none border border-slate-200 bg-white text-left hover:border-primary hover:shadow-sm transition-all group"
                        >
                          ...
                          <div className="min-w-0">
                            <p className="text-[13px] font-bold text-slate-800 group-hover:text-primary transition-colors">{ft.label}</p>
                            <p className="text-[11px] font-medium text-slate-500 line-clamp-1">{ft.description}</p>
                          </div>
                        </button>
                      )
                    })}
