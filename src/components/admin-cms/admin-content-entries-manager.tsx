...
import { 
  ...
  updateAdminContentEntryStatusAction 
...
export function AdminContentEntriesManager({ 
  contentType, 
  initialEntries,
  contentTypeSlug,
}: { 
      ...
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 pb-4 -mx-6 px-6 lg:-mx-10 lg:px-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" className="rounded-none hover:bg-muted" onClick={() => router.push(`/admin/cms`)}>
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight">{contentType?.name}</h1>
              <p className="text-muted-foreground text-sm">Review and publish your global content entries</p>
            </div>
          </div>
          <Button 
                    ...
                    <TableRow key={entry.id} className={cn(
                      ...
                      <TableCell className="text-right pr-6">
                        <div className="flex justify-end gap-1.5 transition-opacity">
                          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-none hover:bg-muted hover:text-orange-500" onClick={() => router.push(`/admin/cms/content/${contentTypeSlug}/edit/${entry.id}`)}>
