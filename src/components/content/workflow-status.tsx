...
export function WorkflowStatusBadge({ status, size = "sm" }: { status: string; size?: "xs" | "sm" | "md" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full font-bold ${cfg.bg} ${cfg.text} ${sizes[size]}`}>
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {Object.entries(entries).map(([key, count]) => {
          ...
          if (!cfg) return null
          return (
            <span key={key} className={`inline-flex items-center gap-1.5 rounded-lg px-2 py-0.5 text-xs font-semibold ${cfg.bg} ${cfg.text}`}>
              <span className={`h-1.5 w-1.5 rounded-full ${cfg.dot}`} />
