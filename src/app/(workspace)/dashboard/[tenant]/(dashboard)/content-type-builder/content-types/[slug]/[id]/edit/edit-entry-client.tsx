...
export default function CTBEditEntryClient({
  tenantSlug,
  contentTypeSlug,
  entryId,
}: {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        setLoading(true)
        const [entryData, locRes, limitRes] = await Promise.all([
        ...
        if (entryData?.error) {
          toast({ variant: "destructive", title: "Terjadi Kesalahan", description: entryData.error })
        } else if (entryData?.entry) {
          ...
          if (entryData.contentType) {
            setContentType(entryData.contentType as any)
          }
        }
        ...
        toast({ variant: "destructive", title: "Terjadi Kesalahan", description: "Gagal memuat rincian entri" })
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSave = async (publishNow: boolean = false, targetOverride?: string) => {
    ...
    try {
      ...
      if (res.error) {
        toast({ variant: "destructive", title: "Gagal Memperbarui", description: res.error })
      } else {
        setEntryStatus(targetStatus)
        toast({ 
          title: publishNow ? "Berhasil Diterbitkan" : "Perubahan Disimpan",
          description: `Entri untuk ${contentType?.name || 'konten'} berhasil diperbarui.`
        })
      }
    } catch (error: any) {
  ...
  const renderField = (field: Field) => {
    ...
    switch (field.type) {
      ...
      case "relation": {
        const isRelMultiple = field.options?.relationType === "oneToMany" || 
