                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                      ...
                      {managedInfra.status !== "cancelled" && (
                        <Button asChild size="sm" className="rounded-xl h-8 text-xs">
                          <Link href={`/dashboard/${tenantSlug}/subscriptions/checkout?plan=managed_byodb_monthly`}>Perpanjang 1 Bulan</Link>
                        </Button>
                      )}
                    </div>
                  ) : (
                    <Button asChild size="sm" className="rounded-xl h-8 text-xs">
                      <Link href={`/dashboard/${tenantSlug}/subscriptions/checkout?plan=managed_byodb_setup`}>Pesan Layanan</Link>
                    </Button>
                  )}
                </CardContent>
              </Card>
