  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.error) {
        ...
        router.push(`/developer/${tenantSlug}/content-type-builder/components`)
      } else {
        ...
        <div className="bg-card/80 backdrop-blur-md border-b border-border/60 px-4 md:px-6 py-3.5 sticky top-0 z-10 shrink-0">
          <div className="flex items-center justify-between max-w-7xl mx-auto w-full">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-black text-foreground">Komponen Baru</h1>
                <p className="text-muted-foreground text-xs">Buat grup field yang dapat digunakan kembali pada berbagai model skema.</p>
              </div>
            </div>
              ...
              <Card className="bg-card text-card-foreground border border-border/80 shadow-xs rounded-2xl overflow-hidden">
                ...
                <CardContent className="p-5 space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input value={slug} onChange={e => setSlug(e.target.value)} placeholder="tombol-cta" className="bg-background border-border/80 rounded-xl h-9 font-mono text-xs" />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Select value={category} onValueChange={setCategory}>
                      ...
                      <SelectContent className="rounded-xl border-border bg-card">
                        {CATEGORIES.map(cat => <SelectItem key={cat} value={cat} className="rounded-lg text-xs">{cat}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-1.5">
