  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 w-full max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <Button asChild className="h-9 px-4 text-xs font-bold rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground shadow-xs transition-all cursor-pointer">
            <Link href={`/developer/${tenantId}/content-type-builder/aiwebsitebuilder`}>
          ...
          <Button variant="outline" asChild className="h-9 px-3 text-xs font-semibold rounded-xl border-border/80 bg-card hover:bg-muted/50 cursor-pointer">
            <Link href={`/developer/${tenantId}/media`}>
        ...
        <Card className="rounded-2xl border-2 border-emerald-500/30 bg-gradient-to-r from-emerald-500/10 via-card to-card p-4 shadow-xs overflow-hidden">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                <Link href={`/developer/${tenantId}/deployments`}>
                  Detail Hosting
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Button variant="ghost" size="sm" asChild className="rounded-xl h-8 px-3 text-xs text-muted-foreground hover:text-foreground">
                <Link href={`/developer/${tenantId}/infrastructure?tab=hosting`}>
                  Detail Hosting
                  <ArrowRight className="h-3 w-3 ml-1" />
                </Link>
              </Button>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            ...
            <Button variant="outline" asChild className="h-9 px-3.5 text-xs font-bold rounded-xl border-border/80 bg-card hover:bg-muted/60 cursor-pointer">
              <Link href={`/developer/${tenantId}/tools/docs`}>
      ...
      {usageAlerts.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xs">
          ...
          <Button asChild size="sm" className="bg-red-600 hover:bg-red-700 text-white font-bold text-xs rounded-xl h-8 px-4 cursor-pointer">
            <Link href={`/developer/${tenantId}/subscriptions`}>
              Upgrade Paket
            </Link>
          </Button>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          { label: "Aset Media", value: stats?.mediaCount ?? 0, icon: ImageIcon, color: "text-blue-500", bg: "bg-blue-500/10", href: `/developer/${tenantId}/media` },
          { label: "Anggota Tim", value: stats?.memberCount ?? 1, icon: Users, color: "text-purple-500", bg: "bg-purple-500/10", href: `/developer/${tenantId}/users` },
          { label: "Member Aplikasi", value: stats?.appMemberCount ?? 0, icon: UserCheck, color: "text-indigo-500", bg: "bg-indigo-500/10", href: `/developer/${tenantId}/users-permissions/members` },
          { label: "Webhook Aktif", value: stats?.webhookCount ?? 0, icon: Webhook, color: "text-amber-500", bg: "bg-amber-500/10", href: `/developer/${tenantId}/tools/webhooks` },
          { label: "API Token", value: stats?.apiTokenCount ?? 0, icon: Key, color: "text-emerald-500", bg: "bg-emerald-500/10", href: `/developer/${tenantId}/tools/api-keys` },
        ].map((kpi) => (
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Link 
                href={schemaView === "collections" ? `/developer/${tenantId}/content-type-builder/content-types` : `/developer/${tenantId}/content-type-builder/single-types`} 
            ...
            {schemaView === "collections" ? (
              contentTypes.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {contentTypes.map((ct) => (
                    <Link key={ct.id} href={`/cms/${tenantId}/content/${ct.slug}`}>
              ...
              singleTypes.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {singleTypes.map((st) => (
                    <Link key={st.id} href={`/cms/${tenantId}/single-types/${st.slug}`}>
                      <Card className="hover:border-primary/40 hover:shadow-md transition-all cursor-pointer group bg-card border border-border/80 rounded-2xl shadow-xs">
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="space-y-6">
              ...
              {(!stats?.recentEntries || stats.recentEntries.length === 0) ? (
                ...
                <div className="divide-y divide-border/60">
                  {stats.recentEntries.map((entry) => {
                    ...
                    return (
                      <Link key={entry.id} href={`/cms/${tenantId}/content/${entry.contentTypeSlug}`}>
          ...
          <Card className="bg-card border border-border/80 rounded-2xl shadow-xs">
            ...
            <CardContent className="space-y-2 p-4 pt-1">
              <Link href={`/developer/${tenantId}/tools/api`}>
              ...
              <Link href={`/developer/${tenantId}/tools/sdk`}>
                <div className="flex items-center gap-3 p-2.5 rounded-xl bg-muted/30 border border-border/60 hover:border-primary/40 transition-all group">
                  <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary">
              ...
              <Link href={`/developer/${tenantId}/tools/mcp`}>
                <div className="flex items-center gap-3 p-2.5 rounded-xl bg-muted/30 border border-border/60 hover:border-primary/40 transition-all group">
                  <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary">
