                ...
                <div className="flex items-center gap-4">
                  ...
                  {subscription?.status === 'trialing' ? (
                    <Button 
                      ...
                      onClick={() => router.push(`/developer/${tenantSlug}/subscriptions/checkout?plan=${subscription?.plan || 'starter'}&interval=year`)}
                    >
                      Bayar Sekarang
                    </Button>
                      ...
                      <CardContent className="p-4 pt-0 space-y-3.5 flex-1 flex flex-col justify-between">
                        ...
                        <div className="pt-2">
                          <Button 
                            ...
                            onClick={() => !isCurrent && router.push(`/developer/${tenantSlug}/subscriptions/checkout?plan=${plan.id}&interval=year`)}
                            disabled={isCurrent}
                          >
                            {isCurrent
                        ...
                        <div className="flex sm:flex-col justify-end gap-2 shrink-0">
                          {addon.priceLabel && addon.price === 0 ? (
                            ...
                            <Button 
                              ...
                              onClick={() => (addon.isTopup || !activeAddons.includes(addon.id)) && router.push(`/developer/${tenantSlug}/subscriptions/checkout?plan=${addon.id}`)}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {inv.isTransaction && inv.status === 'pending' ? (
                            <Button 
                              ...
                              onClick={() => router.push(`/developer/${tenantSlug}/subscriptions/checkout?plan=${inv.plan}&interval=year`)}
                            >
                              Bayar Sekarang
                            </Button>
                          ) : (
        ...
        {services.storageAddons.map((addon) => (
          <div key={addon.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
            ...
            {addon.renewed ? (
              ...
              <Button size="sm" className="rounded-xl h-8 text-xs" onClick={() => router.push(`/developer/${tenantSlug}/subscriptions/checkout?plan=topup_storage_10gb&renew=${addon.id}`)}>
                Perpanjang
              </Button>
            )}
          </div>
        ...
        {services.managedInfra && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
            ...
            {services.managedInfra.status !== "cancelled" && (
              <Button size="sm" className="rounded-xl h-8 text-xs" onClick={() => router.push(`/developer/${tenantSlug}/subscriptions/checkout?plan=managed_byodb_monthly`)}>
                Perpanjang 1 Bulan
              </Button>
            )}
          </div>
