 ...
 *
 * A plain `redirect("/login")` from a Server Component isn't enough to
 * fix that on its own: the stale cookie is still cryptographically valid, so
 * `/login`'s own "already authenticated" check would immediately bounce
...
export async function GET(request: NextRequest) {
  ...
  const response = NextResponse.redirect(
    new URL(`/login?redirect_to=${encodeURIComponent(redirectTo)}`, request.url)
  )
