 ...
 *
 * Replaces the old v0 SDK streaming route (v0/chats/stream) and the Claude
 * REST route (generate-frontend) with a single AI SDK-based endpoint that
 * streams responses from ANY provider (OpenAI, Anthropic, Google).
 *
 * Called by the frontend's `useChat` hook with `@ai-sdk/react` transport.
 * The response is a standard AI SDK text stream, not a v0-specific format.
 */
...
export const POST = withStaffAuth(
  async (req, _context, { access, session }) => {
    ...
    const modelId: string = body?.modelId || body?.model || "gpt-4o"
    ...
    const prompt = typeof lastUserMessage?.content === "string"
      ? lastUserMessage.content.slice(0, 8000)
      : ""
    ...
    if (!isIteration) {
      // Full generation — prepare workspace context
      ...
      try {
        ...
        console.warn("[AI_BUILDER_STREAM] Schema auto-provision via MCP warning:", schemaErr.message)
      }
...
4. Include Navbar, Hero, Feature cards, detail views, Footer.
5. Format currency as Rupiah, use star ratings, badges.
...
4. Keep Tailwind CSS, Lucide icons, and TypeScript types intact.`
    ...
    // ── Resolve model & stream ──
    let model
    try {
      model = await resolveModel(modelId)
    } catch (err: any) {
      return apiError("validation", { message: err.message })
    }

    // Build messages with context injected
    const aiMessages: any[] = contextBlock
      ? [
    ...
    await deductUserAiCredits(session.user.id, creditCost, isIteration ? "iterate_website" : "generate_frontend", tenant.id, modelId)

    // Save settings
    if (!isIteration) {
      const chatId = `sacms_ai_${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 7)}`
      await db.setting.upsert({ where: { key: `${tenant.id}_v0ChatId` }, update: { value: chatId }, create: { tenantId: tenant.id, key: `${tenant.id}_v0ChatId`, value: chatId } })
    ...
    const result = streamText({
      ...
      temperature: 0.3,
      async onFinish({ text }) {
        // Parse and save files after streaming completes
        try {
          ...
          console.warn("[AI_BUILDER_STREAM] Could not parse/save files from stream:", err.message)
        }
      },
    })
...
// ────────────────────────────────────────────────────────────────────────────
// Helpers (duplicated to avoid circular import from website-agent.ts)
// ────────────────────────────────────────────────────────────────────────────
...
async function syncFilesToDb(tenantId: string, tenantSlug: string, files: Array<{ name: string; content: string }>) {
  let site = await db.site.findFirst({ where: { tenantId }, orderBy: { updatedAt: "desc" } })
  if (!site) {
    const tenant = await db.tenant.findUnique({ where: { id: tenantId }, select: { name: true } })
    site = await db.site.create({
      data: { tenantId, name: `${tenant?.name || tenantSlug} Website`, slug: `${tenantSlug}-web`, subdomain: `${tenantSlug}-web`, status: "published" },
    })
  }
