 ...
 * (MCP schema inspection/generation, sample-data injection, SaCMS API token
 * minting, the mega-prompt template) but calls `v0.chats.createStream()`
 * instead of `createV0Chat()`'s create-then-poll pattern, and returns the
 * live SSE response straight through via `.toResponse()` so `@v0-sdk/react`'s
