...
export async function GET(
  ...
  try {
    ...
    // ─── Header ───
    ln(`# AI Frontend Builder Prompt — ${tenant.name}`)
    ln()
    ln(`> This prompt was auto-generated by SaCMS on ${new Date().toISOString().split("T")[0]}.`)
    ln(`> Use this prompt with any AI coding tool (ChatGPT, Claude, Gemini, Cursor, etc.) to generate a complete frontend application.`)
    ln()

    // ─── 1. Project Configuration ───
    ln(`## 1. Project Configuration`)
    ln()
    ln(`| Key | Value |`)
    ln(`|-----|-------|`)
    ln(`| Tenant Name | ${tenant.name} |`)
    ln(`| Tenant Slug | \`${tenantSlug}\` |`)
    ln(`| Base API URL | \`${fullApiBaseUrl}\` |`)
    ln(`| GraphQL Endpoint | \`${fullApiBaseUrl}/graphql\` |`)
    ln(`| Brand API | \`${fullApiBaseUrl}/brand\` (no auth required) |`)
    ln(`| Authentication | \`Authorization: Bearer ${tokenPlaceholder}\` |`)
    ln(`| API Key | \`${tokenPlaceholder}\` — get this from the API Tokens page in the CMS dashboard |`)
    ln()
    ...
    ln(`└── .env.local                  ← SACMS_API_KEY=${tokenPlaceholder}`)
    ln("```")
    ln()
    ...
    ln(`- Store \`SACMS_API_KEY\` in \`.env.local\` — never expose it client-side`)
