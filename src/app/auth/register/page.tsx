...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.autoVerified) {
        ...
        const loginUrl = `/auth/login?email=${encodeURIComponent(formData.email)}${redirectTo ? '&redirect_to=' + encodeURIComponent(redirectTo) : '&redirect_to=' + encodeURIComponent(defaultTarget)}`
  ...
  const loginHref = redirectTo 
    ...
    : `/auth/login${formData.email ? '?email=' + encodeURIComponent(formData.email) : ''}`
