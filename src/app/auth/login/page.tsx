...
export default function LoginPage() {
    ...
    if (status === "authenticated" && session?.user) {
      ...
      if (redirectTo) {
        ...
        if (isSuperAdminOwnerOrAdmin) {
          ...
          router.push(`/cms/${userTenants[0].slug || userTenants[0].id}`)
        } else {
          router.push("/dashboard")
        }
      }
  ...
  useEffect(() => {
    const checkFirstUser = async () => {
      try {
        ...
        if (data.isFirstUser) {
          router.push(redirectTo ? `/auth/register?redirect_to=${encodeURIComponent(redirectTo)}` : "/auth/register")
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (searchParams.get("verified") === "true") {
      ...
      router.replace(redirectTo ? `/auth/login?redirect_to=${encodeURIComponent(redirectTo)}` : "/auth/login")
    }
    ...
    try {
      ...
      if (redirectTo) {
        ...
        if (isSuperAdminOwnerOrAdmin) {
          ...
          destination = `/cms/${userTenants[0].slug || userTenants[0].id}`
        } else {
          destination = "/dashboard"
        }
      }
  ...
  const registerHref = registerParams.toString() ? `/auth/register?${registerParams.toString()}` : "/auth/register"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <Link href="/auth/forgot-password" className="text-xs font-bold text-primary hover:text-primary/80 transition-colors">
                  Lupa kata sandi?
                </Link>
              </div>
