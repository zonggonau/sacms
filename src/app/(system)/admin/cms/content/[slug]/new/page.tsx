...
import { 
  ...
  Clock, Archive, Loader2, Calendar as CalendarIcon, AlertCircle, Check, Plus, Zap
} from "lucide-react"
...
export default function AdminCMSCreateEntryPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!contentTypeSlug || contentType) return
      try {
        const ctData = await getAdminContentTypeBySlugAction(contentTypeSlug)
  ...
  }, [contentTypeSlug, status, contentType])
  ...
  const handleSave = async (publishNow: boolean = false) => {
    ...
    try {
      const res = await createAdminEntryAction(contentTypeSlug, {
        data: formData,
        status: targetStatus,
        locale,
  ...
  const renderField = (field: Field) => {
    ...
    switch (field.type) {
      ...
      case "component":
        ...
        return <ComponentField tenantSlug={"global"} componentSlug={compOpts?.componentSlug} value={value} onChange={v => handleFieldChange(field.slug, v)} label={field.name} repeatable={compOpts?.repeatable} />
