...
import { deleteAdminSingleTypeAction, saveAdminSingleTypeDataAction } from "@/actions/admin-single-types"
...
interface AdminSingleTypesClientProps {
  initialSingleTypes: SingleType[]
}

export function AdminSingleTypesClient({ initialSingleTypes }: AdminSingleTypesClientProps) {
  ...
  const handlePublishToggle = (singleType: SingleType, publish: boolean) => {
    startTransition(async () => {
      const res = await saveAdminSingleTypeDataAction(singleType.id, singleType.data, publish)
      if (res.error) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const res = await deleteAdminSingleTypeAction(deleteDialog.singleType!.id)
      if (res.error) {
        toast({ variant: "destructive", title: "Error", description: res.error })
      } else {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <Button 
                ...
                onClick={() => router.push(`/admin/cms/single-types/new`)}
              >
            ...
            <Card className="bg-white border border-slate-200 rounded-none shadow-sm">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xl font-black">{initialSingleTypes.length}</p>
                </div>
              </CardContent>
            </Card>
            <Card className="bg-white border border-slate-200 rounded-none shadow-sm">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xl font-black">{initialSingleTypes.filter(s => s.publishedAt).length}</p>
                </div>
              </CardContent>
            </Card>
            <Card className="bg-white border border-slate-200 rounded-none shadow-sm">
              ...
              {filteredSingleTypes.length === 0 ? (
                <div className="py-24 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/60 mt-1">
                    {searchTerm ? `No results for "${searchTerm}". Try another keyword.` : "Start by creating a new structure for your global singleton content."}
                  </p>
                </div>
              ) : (
                <Table>
                  ...
                  <TableBody>
                    {filteredSingleTypes.map((st) => (
                      <TableRow key={st.id} className="group hover:bg-muted/5 transition-colors cursor-pointer" onClick={() => router.push(`/admin/cms/single-types/${st.slug}/edit`)}>
                        ...
                        <TableCell className="text-center font-bold text-xs">{st.fields?.length || 0}</TableCell>
                          ...
                          <div className="flex justify-end gap-2 items-center">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                  <DropdownMenuItem onClick={() => router.push(`/admin/cms/single-types/${st.slug}/edit`)}>
                                    <Layout className="mr-2 h-4 w-4" /> Edit Schema
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => router.push(`/admin/cms/single-types/${st.slug}`)}>
                                    <FileText className="mr-2 h-4 w-4" /> Edit Data
                                  </DropdownMenuItem>
                                  <DropdownMenuSeparator />
