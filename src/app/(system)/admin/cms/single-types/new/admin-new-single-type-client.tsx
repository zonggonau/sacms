...
import { createAdminSingleTypeAction } from "@/actions/admin-single-types"

export default function AdminNewSingleTypeClient() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const res = await createAdminSingleTypeAction({
        name,
        slug,
        description,
      ...
      if (!res.error) {
        toast({ title: "Success", description: "Global single type created successfully" })
        router.push(`/admin/cms/single-types`)
      } else {
        ...
        <div className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-10 shrink-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href={`/admin/cms/single-types`}>
