...
import { getAdminSingleTypeBySlugAction } from "@/actions/admin-single-types"

export default async function EditAdminSingleTypePage({
  params,
}: {
  ...
  const singleTypeSlug = resolvedParams.slug

  let initialSingleType = null
  let initialFields = []

  try {
    const response = await getAdminSingleTypeBySlugAction(singleTypeSlug)
