...
import { getAdminSingleTypeBySlugAction, saveAdminSingleTypeDataAction } from "@/actions/admin-single-types"
...
export default function AdminCMSSingleTypeDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const stRes = await getAdminSingleTypeBySlugAction(singleTypeSlug)
  ...
  const handleSave = async (publishNow: boolean = false) => {
    ...
    setSaving(true)
    try {
      const response = await saveAdminSingleTypeDataAction(
        singleType.id, 
        formData, 
        publishNow
      )
  ...
  const renderField = (field: Field) => {
    ...
    switch (field.type) {
      ...
      case "relation":
        let relOpts: any = {}
        if (field.options) {
          try { relOpts = typeof field.options === 'string' ? JSON.parse(field.options) : field.options } catch (e) {}
        }
      ...
      <main className="p-6 lg:p-10 max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => router.push(`/admin/cms`)} className="rounded-none hover:bg-muted">
            ...
            <div>
              ...
              <p className="text-muted-foreground font-medium mt-1">Global Page Content &middot; /{singleType.slug}</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-6 bg-card border border-border rounded-none text-foreground shadow-none relative">
              ...
              <div className="flex items-center gap-2 mb-3">
                <h4 className="font-black uppercase text-xs tracking-widest text-foreground">Global Content</h4>
              </div>
              <p className="text-[11px] leading-relaxed font-medium text-muted-foreground">
                These changes are specific to the <strong>Global</strong> space and may affect the root website content directly.
              </p>
            </div>
          </div>
        </div>
