...
import { createAdminComponentAction } from "@/actions/admin-components"

const CATEGORIES = ["SEO", "Media", "Content", "Layout", "Settings", "Other"]

export default function AdminNewComponentClient() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const res = await createAdminComponentAction({
        name,
        slug,
        description,
        category,
      ...
      if (!res.error) {
        toast({ title: "Success", description: "Global component created successfully" })
        router.push(`/admin/cms/components`)
      } else {
        ...
        <div className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-10 shrink-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href={`/admin/cms/components`}>
