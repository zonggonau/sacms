          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Konfigurasi batas kapabilitas platform antara <strong>Super Admin</strong> dan <strong>Account Owner</strong>.
              </p>
            </div>
            <div className="flex items-center gap-2.5">
              <Button variant="outline" size="sm" onClick={() => { setRefreshing(true); fetchData(); }} disabled={refreshing} className="rounded-xl h-9 text-xs font-bold shadow-xs border-border/80">
                <RefreshCw className={`h-3.5 w-3.5 mr-1.5 ${refreshing ? "animate-spin" : ""}`} /> Muat Ulang
              </Button>
              <Dialog open={isPermOpen} onOpenChange={setIsPermOpen}>
                <DialogTrigger asChild>
                  <Button size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-xs h-9 rounded-xl shadow-xs">
                    <Plus className="mr-1.5 h-3.5 w-3.5" /> Tambah Izin Baru
                  </Button>
                </DialogTrigger>
          ...
          <div className="flex items-start gap-3 p-4 rounded-2xl bg-primary/5 border border-primary/20 text-xs text-foreground">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-muted-foreground leading-relaxed">
                Halaman ini mengelola hak akses di tingkat <strong>Platform Global</strong> (Super Admin vs Account Owner). 
