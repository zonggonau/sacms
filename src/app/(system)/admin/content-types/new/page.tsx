  ...
  const handleSaveSchema = async () => {
    ...
    try {
      const res = await fetch("/api/admin/content-types", {
        ...
        body: JSON.stringify({
          name, slug, description, isPublished,
      ...
      if (res.ok) {
        toast({ title: "Schema Created", description: "Global content type is now available." })
        ...
        <div className="p-6 lg:p-8 w-full space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight uppercase">New Global Schema</h1>
                <p className="text-muted-foreground text-sm">Define a new collection for the entire platform.</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-1 space-y-6">
              <Card className="border-none shadow-sm">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2"><Label className="text-xs font-bold">API Slug</Label><Input value={slug} onChange={e => setSlug(e.target.value)} className="bg-muted/30 border-none font-mono text-xs" /></div>
        ...
        <DialogContent className="max-w-lg rounded-3xl border-none shadow-2xl overflow-hidden p-0">
          <div className="flex flex-col max-h-[90vh]">
            <DialogHeader className="p-6 bg-primary text-primary-foreground shrink-0">
              {(() => {
                const fieldType = editingField && FIELD_TYPES.find(ft => ft.type === editingField.type)
