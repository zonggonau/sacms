...
export default function EditComponentSchemaPage({
  params,
}: {
  ...
  }, [tenantSlug, componentSlug, session])
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/tenant/${tenantSlug}/components/${componentId}`, {
        method: "PATCH",
      ...
      if (res.ok) {
        toast({ title: "Updated", description: "Component structure updated" })
        ...
        <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight uppercase">Edit Component</h1>
                <p className="text-muted-foreground text-sm">{name} &middot; Shared Asset</p>
              </div>
            </div>
            {!isReadOnly && (
          ...
          {isReadOnly && (
            <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center gap-4 text-indigo-700">
              ...
              <p className="text-sm font-medium">Global component. Structure is read-only.</p>
            </div>
          )}
            ...
            <div className="lg:col-span-1 space-y-6">
              <Card className="border-none shadow-sm">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2"><Label className="text-xs font-bold">Display Name</Label><Input value={name} onChange={e => setName(e.target.value)} disabled={isReadOnly} className="bg-muted/30 border-none font-bold" /></div>
          ...
          <div className="flex flex-col max-h-[90vh]">
            ...
            <ScrollArea className="flex-1 overflow-y-auto bg-card">
              <div className="p-6 space-y-6">
                ...
                {editingField?.type === "relation" && <div className="p-4 bg-muted/20 rounded-2xl"><RelationFieldConfig tenantSlug={tenantSlug} context="component" relationType={editingField.relationType} targetModel={editingField.targetModel} targetSlug={editingField.targetSlug} onRelationTypeChange={v => setEditingField(prev => prev ? ({ ...prev, relationType: v }) : null)} onTargetModelChange={v => setEditingField(prev => prev ? ({ ...prev, targetModel: v, targetSlug: "" }) : null)} onTargetSlugChange={v => setEditingField(prev => prev ? ({ ...prev, targetSlug: v }) : null)} /></div>}
                {editingField?.type === "component" && <div className="p-4 bg-muted/20 rounded-2xl"><ComponentFieldConfig tenantSlug={tenantSlug} componentSlug={editingField.componentSlug} repeatable={editingField.repeatable} onComponentSlugChange={v => setEditingField(prev => prev ? ({ ...prev, componentSlug: v }) : null)} onRepeatableChange={v => setEditingField(prev => prev ? ({ ...prev, repeatable: v }) : null)} excludeSlug={slug} /></div>}
