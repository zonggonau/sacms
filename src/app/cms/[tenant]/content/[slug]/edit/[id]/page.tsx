    ...
    async function fetchData() {
      ...
      try {
        ...
        if (entryRes.ok) {
          ...
          let parsedData = data.entry.data
          if (typeof data.entry.data === 'string') {
            try { parsedData = JSON.parse(data.entry.data) } catch (err) { parsedData = {} }
          }
          setFormData(parsedData || {})
          setEntryStatus(data.entry.status)
          setLocale(data.entry.locale || "en")
        }
        if (locRes.ok) {
    ...
    <div className="p-6 lg:p-10 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-emerald-700">Edit {contentType?.name}</h1>
            <p className="text-muted-foreground">Updating content ID: {entryId.substring(0,8)}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => handleSave(true)} disabled={saving} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-200 dark:shadow-none font-bold rounded-xl h-10">
            ...
            {entryStatus === "PUBLISHED" ? "Update Live" : "Publish Now"}
          </Button>
        </div>
      </div>
